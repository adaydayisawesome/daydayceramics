...
/**
 * Per-product asset overrides, keyed by `"<collection-slug>/<product-slug>"`.
 *
 * GENERATED by `tools/asset-pipeline/import.mjs` (`npm run import-assets`) into
 * `product-assets.generated.json`. Empty (a no-op) until real photos/videos are
 * imported, at which point the importer fills in `defaultImage` /
 * `alternateImage` / `spinMedia` / `hoverType` per processed product. Keeping it
 * as a generated data file (rather than rewriting this source) makes imports
 * idempotent and keeps the hand-authored catalog below stable and reviewable.
 */
type ProductAssetOverride = {
  defaultImage?: string;
  alternateImage?: string | null;
  spinMedia?: string | null;
  hoverType?: HoverType;
...
const assetOverrides = generatedAssetOverrides as unknown as Record<
  string,
  ProductAssetOverride
>;
...
/**
 * Layer generated asset overrides onto the hand-authored catalog. Only fields
 * present in an override entry replace the base value, so the importer can
 * update a single product's images without disturbing anything else. With an
 * empty override map this returns the base catalog unchanged.
 */
function withAssetOverrides(cols: Collection[]): Collection[] {
  return cols.map((collection) => ({
    ...collection,
    products: collection.products.map((product) => {
      const override = assetOverrides[`${collection.slug}/${product.slug}`];
      if (!override) return product;
      return {
        ...product,
        defaultImage: override.defaultImage ?? product.defaultImage,
        alternateImage:
          override.alternateImage !== undefined
            ? override.alternateImage
            : product.alternateImage,
        spinMedia:
          override.spinMedia !== undefined
            ? override.spinMedia
            : product.spinMedia,
        hoverType: override.hoverType ?? product.hoverType,
      };
    }),
  }));
}

export const collections: Collection[] = withAssetOverrides(baseCollections);
